...
def run_tests(solve):
    ...
    tests = [
        ...
        {
            "name": "tree structure (level order)",
            "inputs": {"start": g5["A"]},
            "check": lambda r: (
                r[0] == "A" and set(r[1:3]) == {"B", "C"} and set(r[3:5]) == {"D", "E"}
            ),
            "fail_msg": "expected level order: A, then {B,C}, then {D,E}",
        },
        {
        ...
        {
            "name": "star graph",
            "inputs": {"start": g9["center"]},
            "check": lambda r: r[0] == "center"
            and set(r[1:]) == {"a", "b", "c", "d", "e"},
            "fail_msg": "expected center first, then all leaves",
        },
        {
            "name": "diamond graph",
            "inputs": {"start": g10["A"]},
            "check": lambda r: (
                r[0] == "A" and set(r[1:3]) == {"B", "C"} and r[3] == "D"
            ),
            "fail_msg": "expected A, then {B,C}, then D",
        },
        {
            "name": "complete graph K4",
            "inputs": {"start": g11["A"]},
            "check": lambda r: r[0] == "A" and set(r) == {"A", "B", "C", "D"},
