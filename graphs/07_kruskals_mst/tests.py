...
def run_tests(solve):
    ...
    # A-B has edges with weight 5 and 2; B-C has edges with weight 4 and 1
    # MST should pick A-B(2) + B-C(1) = 3
    edges_parallel = [
        ("A", "B", 5), ("A", "B", 2),
        ("B", "C", 4), ("B", "C", 1),
    ]
    ...
    tests = [
        ...
        # 11. Parallel edges — must pick lightest among duplicates
        {
            "name": "parallel edges (pick lightest)",
            "inputs": {
                "nodes": ["A", "B", "C"],
                "edges": edges_parallel,
            },
            "check": lambda r: r == 3,
            "fail_msg": lambda r: f"MST: A-B(2)+B-C(1)=3, got {r}",
        },
        # 12. Edges given in descending order — verify sorting works
        {
            "name": "reverse-sorted edges",
            "inputs": {
                "nodes": ["A", "B", "C", "D"],
                "edges": [
                    ("C", "D", 10), ("B", "C", 5), ("A", "B", 1),
            ...
            "check": lambda r: r == 16,
            "fail_msg": lambda r: f"MST: A-B(1)+B-C(5)+C-D(10)=16, got {r}",
        },
    ]

    run_all("kruskals_mst", tests, solve)
