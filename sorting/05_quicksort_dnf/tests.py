...
MANY_DUPES = [random.choice([1, 2, 3]) for _ in range(500)]
...
def run_tests(solve):
    tests = [
        ...
        {
            "name": "many duplicates (3 values, 500 elements)",
            "inputs": {"nums": MANY_DUPES},
            "check": lambda r: r == sorted(MANY_DUPES),
            "fail_msg": "many-duplicates array not sorted correctly",
        },
        {
    ...
    run_all("quicksort_dnf", tests, solve)
