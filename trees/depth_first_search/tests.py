...
def run_dfs_tests(dfs_func):
    ...
    Args:
        dfs_func: A function that takes (root) and returns traversal order
    """
    print(f"Running tests for {dfs_func.__name__}...\n")

    # Test 1: Empty tree
    assert dfs_func(None) == [], "Test 1 failed: empty tree"
    print("Test 1 passed: Empty tree")

    # Test 2: Single node
    assert dfs_func(Tree(1)) == [1], "Test 2 failed: single node"
    print("Test 2 passed: Single node")

    # Test 3: Left child only
    assert dfs_func(make_tree(1, Tree(2))) == [1, 2], "Test 3 failed"
    print("Test 3 passed: Left child only")

    # Test 4: Right child only
    assert dfs_func(make_tree(1, None, Tree(2))) == [1, 2], "Test 4 failed"
    print("Test 4 passed: Right child only")

    # Test 5: Complete binary tree (3 nodes)
    tree = make_tree(1, Tree(2), Tree(3))
    assert dfs_func(tree) == [1, 2, 3], "Test 5 failed"
    print("Test 5 passed: Complete binary tree (3 nodes)")

    # Test 6: Full binary tree (preorder: root, left subtree, right subtree)
    #         1
    #        / \
    #       2   3
    #      / \ / \
    ...
    assert dfs_func(tree) == [1, 2, 4, 5, 3, 6, 7], "Test 6 failed"
    print("Test 6 passed: Full binary tree (preorder)")

    # Test 7: Left-skewed tree
    tree = make_tree(1, make_tree(2, Tree(3)))
    assert dfs_func(tree) == [1, 2, 3], "Test 7 failed"
    print("Test 7 passed: Left-skewed tree")

    # Test 8: Right-skewed tree
    tree = make_tree(1, None, make_tree(2, None, Tree(3)))
    assert dfs_func(tree) == [1, 2, 3], "Test 8 failed"
    print("Test 8 passed: Right-skewed tree")

    # Test 9: Unbalanced tree (preorder)
    tree = make_tree(1, make_tree(2, make_tree(4, Tree(5))), Tree(3))
    assert dfs_func(tree) == [1, 2, 4, 5, 3], "Test 9 failed"
    print("Test 9 passed: Unbalanced tree")

    # Test 10: String values
    tree = make_tree("a", Tree("b"), Tree("c"))
    assert dfs_func(tree) == ["a", "b", "c"], "Test 10 failed"
    ...
    assert dfs_func(tree) == [1, 2, 3, 4, 5, 6], "Test 11 failed"
    ...
    assert dfs_func(tree) == [1, 2, 4, 8, 5, 3, 6, 7], "Test 12 failed"
    print("Test 12 passed: Larger tree")
